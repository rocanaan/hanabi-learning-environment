...
flags.DEFINE_multi_string(
    'self_gin_files', [],
    'List of paths to self gin configuration files (e.g.'
    '"configs/hanabi_rainbow.gin").')
flags.DEFINE_multi_string(
    'gin_bindings', [],
...
def launch_experiment(agentX,environment,obs_stacker,start_iteration, experiment_checkpointer,my_agent):
  ...
  if agentX == 'RainbowAgent':
    ...
    return_collection = run_experiment.run_experiment(my_agent, environment, start_iteration,
                                  obs_stacker,
                                  experiment_logger, experiment_checkpointer,
                                  checkpoint_save_dir,num_iterations=latest_checkpoint_version+10,
    ...
    for agent_1 in AGENT_CLASSES:
      ...
      for agent_2 in AGENT_CLASSES:
        ...
        if (agent_1 == 'RainbowAgent') and (agent_2 == 'RainbowAgent'):
          """
          #swtich to self-play
          run_experiment.load_gin_configs(FLAGS.self_gin_files, FLAGS.gin_bindings)
          environment = run_experiment.create_environment()
          obs_stacker = run_experiment.create_obs_stacker(environment)
          my_agent = run_experiment.create_agent(environment, obs_stacker,'Rainbow')
          start_iteration, experiment_checkpointer = (
          run_experiment.initialize_checkpointing(my_agent,
                                                  experiment_logger,
                                                  checkpoint_dir,
                                                  checkpoint_save_dir,
                                                  FLAGS.checkpoint_version,
                                                  FLAGS.checkpoint_file_prefix))
          #save result
          return_collection = launch_experiment('RainbowAgent',environment,obs_stacker,start_iteration, experiment_checkpointer,my_agent)
          #back to paired
          run_paired_experiment.load_gin_configs(FLAGS.gin_files, FLAGS.gin_bindings)
          environment = run_paired_experiment.create_environment()
          obs_stacker = run_paired_experiment.create_obs_stacker(environment)
